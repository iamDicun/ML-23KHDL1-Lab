...
import glob
import torch
import re # 👈 Gọi thêm pháp sư Regex nha anh yêu
...
print("Khởi tạo Preprocessor, vui lòng chờ tải mô hình...")

# 1. Define teencode mappings ("vs" is handled separately, do not add here)
extra_teencodes = { 
...
input_dir = 'processed_datasets_hygiene'
output_dir = 'final_datasets_hygiene'
...
# Normalize occurrences of the ambiguous token "vs"
def thuan_hoa_vs(text):
    if not isinstance(text, str): 
        return ""
    # Replace cases that clearly indicate "vệ sinh" (hygiene)
    ...
    # Convert remaining standalone "vs" tokens to "với"
...
for file_path in csv_files:
    ...
    try:
        ...
        # Normalize "vs" before preprocessing
        print("Đang xử lý token 'vs'...")
        ...
        print(f"Đang xử lý {len(raw_reviews)} dòng review...")
        ...
        out_path = os.path.join(output_dir, f"final_{file_name}")
...
print("\nHoàn tất xử lý dữ liệu (hygiene).")